    ...
    on<PinEvent>((event, emit) async {
      switch (event.type) {
        case PinEventType.oldPinEntered:
          emit(state.copyWith(oldPin: event.pin));
          break;
        case PinEventType.newPinChosen:
          emit(state.copyWith(newPin: event.pin));
          break;
        case PinEventType.newPinConfirmed:
          final changePinEvent = await repo.changePin(
            state.oldPin,
            state.newPin,
          );
          if (changePinEvent is ChangePinSuccessEvent) {
            emit(state.copyWith(newPinConfirmed: ValidationState.valid));
