...
class DisclosurePermissionBloc
    extends Bloc<DisclosurePermissionBlocEvent, DisclosurePermissionBlocState> {
  final int sessionID;
  final Function(CredentialType) onObtainCredential;

  final IrmaRepository
  _repo; // Repository is hidden by design, because behaviour should be triggered via bloc events.
  ...
  @override
  Stream<DisclosurePermissionBlocState> mapEventToState(
    ...
    if (state is DisclosurePermissionIntroduction &&
      ...
      final selectedConIndices = state.selectedConIndices.map(
        (i, selected) => MapEntry(
          i,
          i == state.currentDiscon!.key ? event.conIndex : selected,
        ),
      );
      ...
      yield DisclosurePermissionIssueWizard(
        plannedSteps: _calculatePlannedSteps(
          state.candidates,
          selectedConIndices,
          session,
        ),
      ...
      if (state.isCompleted) {
        ...
        final choices = candidates.map(
          (i, discon) => MapEntry(
            i,
            Con(
              discon[_findSelectedConIndex(discon)]
        ...
        final prevAddedChoices = {
          for (final choice in choices.entries)
            if (choice.value.any((cred) => cred.previouslyAdded))
              choice.key: Con(
                choice.value.where((cred) => cred.previouslyAdded),
              ),
        };
          ...
          yield DisclosurePermissionChoicesOverview(
            ...
            requiredChoices: Map.fromEntries(
              choices.entries.where(
                (entry) =>
                    candidates[entry.key]!.every((con) => con.isNotEmpty),
              ),
            ),
            optionalChoices: Map.fromEntries(
              choices.entries.where(
                (entry) =>
                    ...
                    candidates[entry.key]!.any((con) => con.isEmpty),
              ),
            ),
            changeableChoices: changeableChoices,
            hasAdditionalOptionalChoices: choices.values.any(
              (choice) => choice.isEmpty,
            ),
            signedMessage: session.isSignatureSession ?? false
        ...
        final credentialsToObtain = state.getSelectedCon(
          state.currentDiscon!.key,
        );
      ...
      final choices = Map.fromEntries(
        session.disclosuresCandidates!.mapIndexed((i, rawDiscon) {
          ...
          final choice = _findSelectedConIndex(
            discon,
            prevChoice: state.choices[i],
          );
          return MapEntry(
            i,
            Con(discon[choice].whereType<ChoosableDisclosureCredential>()),
          );
        }),
      );
      yield DisclosurePermissionChoicesOverview(
        ...
        requiredChoices: Map.fromEntries(
          choices.entries.where(
            (entry) => session.disclosuresCandidates![entry.key].every(
              (con) => con.isNotEmpty,
            ),
          ),
        ),
        optionalChoices: Map.fromEntries(
          choices.entries.where(
            (entry) =>
                entry.value.isNotEmpty &&
                session.disclosuresCandidates![entry.key].any(
                  (con) => con.isEmpty,
                ),
          ),
        ),
      ...
      final discon = _parseCandidatesDisCon(
        session.disclosuresCandidates![event.disconIndex],
      );
      ...
      final prevAddedDiscon = [
        for (final con in discon)
          if (con.isEmpty ||
              con.any((cred) => prevAddedCredTypeIds.contains(cred.fullId)))
            Con(
              con.where((cred) => prevAddedCredTypeIds.contains(cred.fullId)),
            ),
      ];
      ...
      yield DisclosurePermissionChangeChoice(
        ...
        selectedConIndex: _findSelectedConIndex(
          discon,
          prevChoice: state.choices[event.disconIndex],
        ),
      );
      ...
      yield DisclosurePermissionChangeChoice(
        ...
        event is DisclosurePermissionNextPressed) {
      if (state.selectedCon.any(
        (cred) => cred is TemplateDisclosureCredential,
      )) {
        ...
        event is DisclosurePermissionPreviousPressed) {
      yield state.parentState;
    } else if (state is DisclosurePermissionWrongCredentialsObtained &&
        event is DisclosurePermissionDialogDismissed) {
      yield state.parentState;
      ...
      if (state.isSelectedChoosable) {
        ...
        event is DisclosurePermissionAddOptionalDataPressed) {
  ...
  DisclosurePermissionBlocState _mapSessionStateToBlocState(
    ...
    if (session.status != SessionStatus.requestDisclosurePermission) {
      ...
      if (state is DisclosurePermissionIssueWizard) {
        ...
        issueWizardState =
            (state as DisclosurePermissionCredentialInformation).parentState
                as DisclosurePermissionIssueWizard;
      }
      ...
      return refreshedState.isCompleted
          ? refreshedState
          : _validateCredentialsObtained(
              refreshedState.currentCon!,
              refreshedState,
            );
    } else if (state is DisclosurePermissionObtainCredentials ||
        (state is DisclosurePermissionCredentialInformation &&
      ...
      if (state is DisclosurePermissionObtainCredentials) {
        ...
        obtainCredentialsState =
            (state as DisclosurePermissionCredentialInformation).parentState
                as DisclosurePermissionObtainCredentials;
      }

      final refreshedState = _refreshObtainedCredentials(
        obtainCredentialsState,
        session,
      );
      ...
      return refreshedState.allObtained
          ? refreshedState
          : _validateCredentialsObtained([
              refreshedState.currentIssueWizardItem!,
            ], refreshedState);
    } else if (state is DisclosurePermissionChangeChoice ||
        (state is DisclosurePermissionCredentialInformation &&
      ...
      if (state is DisclosurePermissionChangeChoice) {
        ...
        changeChoiceState =
            (state as DisclosurePermissionCredentialInformation).parentState
                as DisclosurePermissionChangeChoice;
      }

      final discon = _refreshDisCon(
        changeChoiceState.disconIndex,
        changeChoiceState.discon,
        session,
      );
      ...
      if (state is DisclosurePermissionAddOptionalData) {
        ...
        optionalDataState =
            (state as DisclosurePermissionCredentialInformation).parentState
                as DisclosurePermissionAddOptionalData;
      }

      return _generateAddOptionalDataState(
        session: session,
        parentState: optionalDataState.parentState,
        alreadyAddedOptionalDisconIndices:
            optionalDataState.parentState.optionalChoices.keys,
      ...
      final requiredChoices = state.requiredChoices.map(
        (i, _) => MapEntry(i, choices[i]!),
      );
      final optionalChoices = state.optionalChoices.map(
        (i, _) => MapEntry(i, choices[i]!),
      );
      ...
      final candidates = Map.fromEntries(
        session.disclosuresCandidates!.mapIndexed(
          (i, rawDiscon) => MapEntry(i, _parseCandidatesDisCon(rawDiscon)),
        ),
      );
      ...
      if (choices.values.every((choice) => choice != null)) {
        final nonNullChoices = choices.map(
          (i, choice) => MapEntry(i, Con(choice!)),
        );
        final requiredChoices = Map.fromEntries(
          nonNullChoices.entries.where(
            (entry) => candidates[entry.key]!.every((con) => con.isNotEmpty),
          ),
        );
        ...
        return DisclosurePermissionChoicesOverview(
          ...
          optionalChoices: optionalChoices,
          changeableChoices: changeableChoices,
          hasAdditionalOptionalChoices:
              choices.keys.length >
              requiredChoices.length + optionalChoices.length,
          signedMessage: session.isSignatureSession ?? false
              ? session.signedMessage
              : null,
        );
            ...
            if (choices[entry.key] == null)
              entry.key: DisCon(
                entry.value
                    .map(
                      (con) =>
                          Con(con.whereType<TemplateDisclosureCredential>()),
                    )
                    .where(
                      (con) => con.every(
                        (cred) => entry.value.flattened.none(
                          (cred2) =>
                              cred2 is ChoosableDisclosureCredential &&
                              cred2.fullId == cred.fullId,
                        ),
                      ),
                    ),
              ),
        ...
        final groupedIssueWizardCandidates =
            groupBy<MapEntry<int, DisCon<TemplateDisclosureCredential>>, bool>(
              issueWizardCandidates.entries,
              (candidateEntry) => candidateEntry.value.every(
                (con) => con.any((cred) => !cred.obtainable),
              ),
            );
        final List<MapEntry<int, DisCon<TemplateDisclosureCredential>>>
        issueWizardCandidatesList = [
          ...(groupedIssueWizardCandidates[true] ?? []),
          ...(groupedIssueWizardCandidates[false] ?? []),
        ];

        final selectedConIndices = issueWizardCandidates.map(
          (i, choice) => MapEntry(i, _findSelectedConIndex(choice)),
        );
        final obtained = issueWizardCandidates.map(
          (i, _) => MapEntry(i, false),
        );
        return DisclosurePermissionIssueWizard(
          plannedSteps: _calculatePlannedSteps(
            issueWizardCandidates,
            selectedConIndices,
            session,
          ),
    ...
    final obtained = prevState.candidates.map(
      (i, prevDiscon) => MapEntry(
        ...
        prevState.obtained[i]! ||
            candidates[i]!.any(
              (con) =>
                  con.every((cred) => cred is ChoosableDisclosureCredential),
            ),
      ),
    );
  ...
  DisclosurePermissionBlocState _validateCredentialsObtained(
    ...
    final credentials = _newlyAddedCredentialHashes.expand(
      (hash) => _repo.credentials.containsKey(hash)
          ? [_repo.credentials[hash]!]
          : <Credential>[],
    );
    ...
    for (final template in templates) {
      ...
      final matchingCred = credentials.firstWhereOrNull(
        (cred) => template.matchesCredential(cred),
      );
      ...
      final templateWithoutValueConstraints = template
          .copyWithoutValueConstraints();
  ...
  DisclosurePermissionObtainCredentials _refreshObtainedCredentials(
    ...
    final obtained = prevState.templates
        .map(
          (template) => newlyAddedCredentials.any(
            (cred) => template.matchesCredential(cred),
          ),
        )
        .toList();
  ...
  DisCon<DisclosureCredential> _refreshDisCon(
    ...
    final includedCredTypeIds = prevDiscon
        .expand((con) => con.map((cred) => cred.fullId))
        .toSet();
    return DisCon(
      discon.fold([], (prev, con) {
        if (con.isEmpty ||
            con.any((cred) => includedCredTypeIds.contains(cred.fullId))) {
          final filteredCon = Con(
            con.where((cred) => includedCredTypeIds.contains(cred.fullId)),
          );
  ...
  DisclosurePermissionChoices _refreshChoices(
    ...
    if (selectedConIndex != null) {
      selectedCon = Con(
        discon[selectedConIndex].whereType<ChoosableDisclosureCredential>(),
      );
    ...
    final requiredChoices = selectedCon != null
        ? prevState.requiredChoices.map(
            (i, con) => MapEntry(i, i == disconIndex ? selectedCon! : con),
          )
        : Map.of(prevState.requiredChoices);
    final optionalChoices = selectedCon != null
        ? prevState.optionalChoices.map(
            (i, con) => MapEntry(i, i == disconIndex ? selectedCon! : con),
          )
    ...
    } else if (!requiredChoices.containsKey(disconIndex) &&
        !optionalChoices.containsKey(disconIndex)) {
    ...
    final hasAdditionalOptionalChoices =
        requiredChoices.length + optionalChoices.length < numberOfDiscons;
  ...
  DisCon<DisclosureCredential> _parseCandidatesDisCon(
    DisCon<DisclosureCandidate> rawDiscon,
  ) {
      ...
      rawDiscon.map((rawCon) {
        ...
        return Con(
          groupedCon.entries.map((entry) {
            final credential =
                _repo.credentials[entry.value.first.credentialHash];
                ...
                .map(
                  (candidate) => Attribute.fromCandidate(
                    ...
                    credential?.attributes
                        .firstWhereOrNull(
                          (attr) => attr.attributeType.fullId == candidate.type,
                        )
                        ?.value,
                  ),
                )
            ...
            if (credential == null ||
                entry.value.any((attr) => attr.notRevokable)) {
              ...
              return ChoosableDisclosureCredential(
                ...
                attributes: attributes,
                previouslyAdded: !_newlyAddedCredentialHashes.contains(
                  credential.hash,
                ),
  ...
  int _findSelectedConIndex(
    ...
    int currSelected = -1;
    if (prevChoice != null) {
      currSelected = discon.indexWhere(
        (con) =>
            prevChoice.every((prevCred) => con.any((cred) => cred == prevCred)),
      );
      if (keepValidPrevChoice &&
          currSelected >= 0 &&
          prevChoice.every(
            (cred) => cred is ChoosableDisclosureCredential && cred.valid,
          )) {
        return currSelected;
      }
    }
    ...
    if (prevDiscon != null) {
      final recentlyAddedCredentialHashes = _newlyAddedCredentialHashes.reversed
          .where(
            (hash) => prevDiscon.flattened
                .whereType<ChoosableDisclosureCredential>()
                .none((cred) => cred.credentialHash == hash),
          );
      final choice = discon.indexWhere(
        (con) => con.any(
          (cred) =>
              cred is ChoosableDisclosureCredential &&
              recentlyAddedCredentialHashes.any(
                (hash) => cred.credentialHash == hash,
              ),
        ),
      );
  ...
  List<DisclosurePermissionStepName> _calculatePlannedSteps(
    ...
    final hasPrevAddedCreds =
        ...
        candidates.entries.any(
          (disconEntry) => session
              .disclosuresCandidates![disconEntry
                  .key][selectedConIndices[disconEntry.key]!]
              .any(
                (candidate) =>
                    candidate.credentialHash.isNotEmpty &&
                    !_newlyAddedCredentialHashes.contains(
  ...
  Stream<DisclosurePermissionBlocState> _obtainCredentials(
    ...
    final selectedConTemplates = selectedCon
