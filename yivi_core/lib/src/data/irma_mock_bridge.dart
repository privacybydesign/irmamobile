...
class IrmaMockBridge extends IrmaBridge {
  ...
  Event _constructRequestVerificationPermissionEvent(
    ...
    final disclosureCandidates = condiscon
        .map(
          (discon) => discon.expand((con) {
            ...
            final nonSingletonCredIds = groupedAttrs.keys.where(
              (credId) =>
                  !_irmaConfiguration.credentialTypes[credId]!.isSingleton,
            );
            ...
            for (final entry in groupedAttrs.entries) {
                ...
                if (c.fullId == entry.key &&
                    entry.value.every(
                      (attrId) =>
                          con[attrId] == null ||
                          con[attrId] ==
                              TextValue.fromRaw(c.attributes[attrId]!).raw,
                    )) {
                  credCandidates.add(
                    entry.value
                        .map(
              ...
              if (disconCandidates.isEmpty) {
                ...
                disconCandidates = disconCandidates
                    .expand(
                      (conCandidate) =>
                          credCandidates.map((cc) => [...conCandidate, ...cc]),
                    )
                    .toList();
              }
            }
                  ...
                  (conCandidates) => con.entries
                      .map(
                        (entry) => conCandidates.firstWhere(
                          ...
                          orElse: () => _generatePlaceholderCandidate(
                            entry.key,
                            entry.value,
                          ),
                        ),
            ...
            final placeholderCandidates = groupedAttrs.map(
              (credId, attrIds) => MapEntry(
                ...
                attrIds
                    .map(
                      (attrId) =>
                          _generatePlaceholderCandidate(attrId, con[attrId]),
                    )
                    .toList(),
              ),
            );
            ...
            if (nonSingletonCandidate.credentialHash.isNotEmpty) {
              final placeholderCandidate = disconCandidates.first
                  .map(
                    (attr) => placeholderCandidates[nonSingletonCredIds.first]!
                        .firstWhere(
        ...
        .map((discon) {
          ...
          final choosableCandidates = discon.where(
            (con) => con.isEmpty || con[0].credentialHash.isNotEmpty,
          );
          final templateCandidates = discon.where(
            (con) => con.isNotEmpty && con[0].credentialHash.isEmpty,
          );
    ...
    return RequestVerificationPermissionSessionEvent(
      ...
      satisfiable: disclosureCandidates.every(
        (discon) => discon.any(
          (con) =>
              con.every((candidate) => candidate.credentialHash.isNotEmpty),
        ),
      ),
  ...
  DisclosureCandidate _generatePlaceholderCandidate(
    String type,
    String? value,
  ) => DisclosureCandidate(
    type: type,
    value: value == null
        ? const TranslatedValue.empty()
        : TextValue(
            translated: TranslatedValue.fromString(value),
            raw: value,
          ).toRaw(),
  );
  ...
  Future<void> mockDisclosureSession(
    ...
    await _sessionEventsSubject.firstWhere(
      (event) => event is NewSessionEvent && event.sessionID == sessionId,
    );
    ...
    yield* events
        .takeUntil(
          _sessionEventsSubject.where(
            (e) => e is RespondPermissionEvent && e.sessionID == sessionId,
          ),
        )
        .where((e) => e is CredentialsEvent)
        .cast<CredentialsEvent>()
        .map(
          (e) => _constructRequestVerificationPermissionEvent(
            sessionId,
            condiscon,
            e.credentials,
            signedMessage,
  ...
  Future<void> mockIssuanceSession(
    ...
    final issuedCredentials = credentials.mapIndexed((i, attrs) {
      ...
      assert(
        attrs.entries
                .map(
                  (attr) => _irmaConfiguration
                      .attributeTypes[attr.key]!
                      .fullCredentialId,
                )
                .toSet()
                .length ==
      ...
      final attributeType =
          _irmaConfiguration.attributeTypes[attrs.keys.first]!;
