...
import "package:yivi_core/src/models/attribute.dart";
...
void main() {
  ...
  test("issuance-in-disclosure", () async {
    ...
    final disclosureSessionStream = repo
        .getSessionState(42)
        .asBroadcastStream()
    ...
    IrmaSessionState disclosureSession = await disclosureSessionStream
        .firstWhere(
          (session) =>
              session.status == SessionStatus.requestDisclosurePermission,
        );
    ...
    expect(
      disclosureSession.disclosuresCandidates![0][0][0].type,
      "irma-demo.IRMATube.member.id",
    );
    expect(
      disclosureSession.disclosuresCandidates![0][0][0].credentialHash,
      "",
    );
    ...
    final issuanceSessionStream = repo
        .getSessionState(43)
        .asBroadcastStream()
        .map((state) => state as IrmaSessionState);

    // Check whether the pairing status is being triggered.
    await issuanceSessionStream.firstWhere(
      (session) => session.status == SessionStatus.pairing,
    );
    ...
    await issuanceSessionStream.firstWhere(
      (session) => session.status == SessionStatus.success,
    );

    disclosureSession = await disclosureSessionStream.firstWhere(
      (session) => session.satisfiable ?? false,
    );
    ...
    expect(
      disclosureSession.disclosuresCandidates![0][1][0].type,
      "irma-demo.IRMATube.member.id",
    );
    expect(
      disclosureSession.disclosuresCandidates![0][1][0].credentialHash,
      "",
    );
    repo.bridgedDispatch(
      RespondPermissionEvent(
        ...
        disclosureChoices: [
          [
            AttributeIdentifier(
              ...
              credentialHash: disclosureSession
                  .disclosuresCandidates![0][0][0]
                  .credentialHash,
            ),
          ],
        ],
    ...
    expect(
      disclosureSession.disclosureChoices?[0][0].type,
      "irma-demo.IRMATube.member.id",
    );
    expect(
      disclosureSession.disclosureChoices?[0][0].credentialHash,
      "session-43-0",
    );
  });
  ...
  test("issuance-in-disclosure-using-specific-attributes", () async {
    ...
    final firstIssuanceSessionStream = repo
        .getSessionState(43)
        .asBroadcastStream()
        .map((state) => state as IrmaSessionState);
    await firstIssuanceSessionStream.firstWhere(
      (session) => session.status == SessionStatus.requestIssuancePermission,
    );
    repo.bridgedDispatch(
      RespondPermissionEvent(
    ...
    final secondIssuanceSessionStream = repo
        .getSessionState(44)
        .asBroadcastStream()
        .map((state) => state as IrmaSessionState);
