...
void main() {
  group("searchCredentials — prefix matching (returns 1.0)", () {
    ...
    test("non-first-token prefix matches (the tokenisation win)", () {
    ...
    test("case-insensitive — query lowercased before matching", () {
      final results = searchCredentials([_entry("driving licence")], "DRIV");
      expect(_hashes(results), ["driving licence"]);
    });

    test("hyphens in the query are stripped", () {
      // "e-mail" → "email" matches the "email" token.
      final results = searchCredentials([_entry("verified email")], "e-mail");
      expect(_hashes(results), ["verified email"]);
    });

    test("whitespace in the query is trimmed", () {
      final results = searchCredentials([_entry("paspoort")], "  pasp  ");
  ...
  group("searchCredentials — fuzzy matching (edit distance)", () {
    ...
    test("realistic mid-word typo passes", () {
      ...
      final results = searchCredentials([_entry("rijbewijs")], "rijbewjs");
      expect(_hashes(results), ["rijbewijs"]);
    });

    test("unrelated long query is rejected once distance exceeds 2", () {
    ...
    test("aggressively scrambled query is rejected", () {
      final results = searchCredentials([_entry("paspoort")], "xxyyzz");
      expect(results, isEmpty);
    });
    ...
    test("adjacent transposition counts as a single edit", () {
      ...
      final results = searchCredentials([_entry("paspoort")], "papsoort");
      expect(_hashes(results), ["paspoort"]);
    });

    test("three typos are rejected (hard 2-edit cap)", () {
      // "xxxpoort" is 3 substitutions from "paspoort" — over the cap.
      final results = searchCredentials([_entry("paspoort")], "xxxpoort");
      expect(results, isEmpty);
    });
  });
  ...
  group("searchCredentials — issuer name as a fallback", () {
    test("query matching only issuer name still ranks the candidate", () {
    ...
    test("credential name match outranks issuer match for the same query", () {
  ...
  group("searchCredentials — result priority", () {
    ...
    test("combined match (cred + issuer) wins; weakest passing match last", () {
      final results = searchCredentials([
        SearchEntry(hash: "a", credentialType: "licence", issuerName: "yivi"),
        SearchEntry(
          hash: "b",
  ...
  group("searchCredentials — edge cases", () {
    ...
    test("blank issuer name doesn't crash", () {
      final results = searchCredentials([
        SearchEntry(hash: "x", credentialType: "paspoort", issuerName: ""),
      ], "pasp");
      expect(_hashes(results), ["x"]);
    });

    test("very short tokens don't match unrelated long queries", () {
      ...
      final results = searchCredentials([_entry("ver")], "overheid");
      expect(results, isEmpty);
    });
  });
  ...
  group("searchCredentials — issuer name interactions", () {
    ...
    test("issuer-only prefix hit (1.0 × 0.3 = 0.3) passes the floor", () {
      // The query doesn't touch the credential name at all; the issuer
      // prefix is what carries the candidate over the bar.
      final results = searchCredentials([_entry("paspoort", "rdw")], "rdw");
      expect(_hashes(results), ["paspoort"]);
    });
    ...
    test("credential prefix + issuer prefix combine to score 1.0", () {
    ...
    test("issuer-only 2-edit fuzzy is filtered (weighted below the floor)", () {
      ...
      final pass = searchCredentials([_entry("paspoort", "rdw")], "rdww");
      expect(_hashes(pass), ["paspoort"]);

      final fail = searchCredentials([_entry("paspoort", "rdw")], "rdwww");
      expect(fail, isEmpty);
    });
    ...
    test("weak issuer-only fuzzy match is rejected", () {
      ...
      final results = searchCredentials([_entry("paspoort", "yivi")], "wivx");
      expect(results, isEmpty);
    });

    test("credential contains outranks issuer prefix for the same query", () {
    ...
    test("query spanning two issuer tokens matches via joined-prefix", () {
    ...
    test("joined-prefix does NOT degrade into joined-contains", () {
  ...
  group("searchCredentials — more edge cases", () {
    ...
    test("query much longer than any token returns empty", () {
    ...
    test("credential name and issuer name independently tokenised", () {
