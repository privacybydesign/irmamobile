...
import "package:yivi_core/src/screens/reset_pin/reset_pin_screen.dart";
  ...
  group("irma-login", () {
    ...
    testWidgets("wrong-pin", (tester) async {
      ...
      String string = tester
          .getAllText(find.byKey(const Key("irma_dialog_title")))
          .first;
      expect(string, "PIN incorrect");

      // Check dialog text
      string = tester
          .getAllText(find.byKey(const Key("irma_dialog_content")))
          .first;
      expect(
        string,
    ...
    testWidgets("blocked-pin", (tester) async {
      ...
      expect(
        string,
        "Your app has been blocked for 1 minute. Please try again later.",
      );
      await tester.tapAndSettle(
        find.descendant(
          of: dialogFinder,
    ...
    testWidgets("reset-from-forgot-pin", (tester) async {
      ...
      final confirmDeleteButtonFinder = find
