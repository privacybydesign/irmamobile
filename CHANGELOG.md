...
- On iOS the app is no longer covered by a full-screen blur while the NFC reader sheet is up, so the scanning animation and its progress text stay readable for the whole read. The blur that hides the wallet in the app switcher is unchanged
...
- Flows that put system UI in front of the app, the iOS NFC reader sheet and the OS biometric prompt, suspend the privacy screen for their duration instead of switching it off and restoring it from the screenshot preference afterwards. Suspensions are counted, are undone even when the flow throws, and leave the preference alone; backgrounding the app during such a flow still blurs
