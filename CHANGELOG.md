...
- Decode the per-attribute `intent_to_retain` flag that mdoc verifiers send and the `display_is_fallback` flag on credentials, so they are available to the app; neither is shown yet
