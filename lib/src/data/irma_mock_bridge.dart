...
class IrmaMockBridge extends IrmaBridge {
  ...
  Event _constructRequestVerificationPermissionEvent(
        ...
        .map(
          (discon) => discon.expand((con) {
            ...
            final nonSingletonCredIds = groupedAttrs.keys.where(
              (credId) => !_irmaConfiguration.credentialTypes[credId]!.isSingleton,
            );
            ...
            for (final entry in groupedAttrs.entries) {
              ...
              for (final RawCredential c in credentials) {
                if (c.fullId == entry.key &&
                    entry.value.every(
                      (attrId) => con[attrId] == null || con[attrId] == TextValue.fromRaw(c.attributes[attrId]!).raw,
                    )) {
                  credCandidates.add(
                    entry.value
                        .map(
                          (attrId) => DisclosureCandidate(
                            type: attrId,
            ...
            disconCandidates = disconCandidates
                .map(
                  (conCandidates) => con.entries
                      .map(
                        (entry) => conCandidates.firstWhere(
                          (candidate) => candidate.type == entry.key,
            ...
            final placeholderCandidates = groupedAttrs.map(
              (credId, attrIds) => MapEntry(
                credId,
                attrIds.map((attrId) => _generatePlaceholderCandidate(attrId, con[attrId])).toList(),
              ),
            );
            ...
            final nonSingletonCandidate = disconCandidates.first.firstWhere(
              (candidate) => candidate.type.startsWith('${nonSingletonCredIds.first}.'),
            );
            if (nonSingletonCandidate.credentialHash.isNotEmpty) {
              final placeholderCandidate = disconCandidates.first
                  .map(
                    (attr) => placeholderCandidates[nonSingletonCredIds.first]!.firstWhere(
                      (phAttr) => phAttr.type == attr.type,
                      orElse: () => attr,
                    ),
                  )
                  .toList();
    ...
    return RequestVerificationPermissionSessionEvent(
      ...
      satisfiable: disclosureCandidates.every(
        (discon) => discon.any((con) => con.every((candidate) => candidate.credentialHash.isNotEmpty)),
      ),
  ...
  Future<void> mockDisclosureSession(
    ...
    yield StatusUpdateSessionEvent(sessionID: sessionId, action: 'disclosing', status: 'communicating');
    yield StatusUpdateSessionEvent(sessionID: sessionId, action: 'disclosing', status: 'connected');
    ...
    yield* events
        ...
        .map((e) => _constructRequestVerificationPermissionEvent(sessionId, condiscon, e.credentials, signedMessage));
  ...
  Future<void> mockIssuanceSession(
    ...
    await _sessionEventsSubject.firstWhere((event) => event is NewSessionEvent && event.sessionID == sessionId);
    yield StatusUpdateSessionEvent(sessionID: sessionId, action: 'issuing', status: 'communicating');
    yield PairingRequiredSessionEvent(sessionID: sessionId, pairingCode: '1234');
    yield StatusUpdateSessionEvent(sessionID: sessionId, action: 'issuing', status: 'connected');
    ...
    final issuedCredentials = credentials.mapIndexed((i, attrs) {
      ...
      assert(
        attrs.entries.map((attr) => _irmaConfiguration.attributeTypes[attr.key]!.fullCredentialId).toSet().length == 1,
      );
    ...
    final permissionEvent =
        await _sessionEventsSubject.firstWhere((e) => e is RespondPermissionEvent && e.sessionID == sessionId)
            as RespondPermissionEvent;
