...
class HistoryState {
  ...
  HistoryState({List<LogEntry> logEntries = const [], this.loading = false, this.moreLogsAvailable = false})
    : logEntries = UnmodifiableListView(logEntries);

  HistoryState copyWith({List<LogEntry>? logEntries, bool? loading, bool? moreLogsAvailable}) {
    return HistoryState(
    ...
    _historyStateSubscription = repo
        ...
        .scan<HistoryState>((prevState, event, _) {
          if (event is LoadLogsEvent) {
            return prevState.copyWith(loading: true, logEntries: event.before == null ? [] : prevState.logEntries);
            ...
            final supportedLogEntries = event.logEntries.where(
              (entry) => _serverNameOptional.contains(entry.type) || entry.serverName != null,
            );
