...
class ActivityDetailRemoval extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final removedCredentials = logEntry.removedCredentials.entries
        .map(
          (entry) => CredentialView.fromRawAttributes(irmaConfiguration: irmaConfiguration, rawAttributes: entry.value),
        )
        .toList();
    ...
    return Column(
      ...
      children: [
        TranslatedText('activity.deleted_data', style: theme.themeData.textTheme.headlineMedium, isHeader: true),
        SizedBox(height: theme.smallSpacing),
        for (var removedCredential in removedCredentials)
          Padding(
            padding: EdgeInsets.only(top: theme.smallSpacing),
            child: IrmaCredentialCard(credentialView: removedCredential, hideFooter: true),
          ),
      ],
    );
  }
