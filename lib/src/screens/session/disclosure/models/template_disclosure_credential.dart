...
class TemplateDisclosureCredential extends DisclosureCredential {
  TemplateDisclosureCredential({required super.info, required List<Attribute> super.attributes});

  bool matchesCredential(Credential credential) =>
      credentialType.fullId == credential.info.fullId &&
      attributes.every(
        (templAttr) => credential.attributes.any(
          (credAttr) =>
  ...
  TemplateDisclosureCredential copyWithoutValueConstraints() => TemplateDisclosureCredential(
    info: info,
    attributes: attributes.map((attr) => Attribute(attributeType: attr.attributeType, value: NullValue())).toList(),
  );
}
