...
class IrmaBottomBar extends StatelessWidget {
  ...
  List<Widget> buildBtns(BuildContext context, BoxConstraints constraints) {
    final List<Widget> btns = [];

    double btnWidth = constraints.maxWidth - 2 * IrmaTheme.of(context).defaultSpacing;
    if (secondaryBtnLabel != null) {
      btnWidth = constraints.maxWidth / 2 - 2 * IrmaTheme.of(context).defaultSpacing;
    ...
    if (tooltipOnPrimaryBtn) {
      primaryBtn = _IrmaToolTip(
      ...
      return Container(
        ...
        child: Padding(
          ...
          child: Wrap(
            ...
            children: buildBtns(context, constraints),
          ),
        ),
      );
    });
