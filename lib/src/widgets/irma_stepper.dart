...
class IrmaStepper extends StatelessWidget {
  ...
  Widget _buildItem(IrmaThemeData theme, int index) => TimelineTile(
    ...
    indicatorStyle: IndicatorStyle(
      ...
      padding: EdgeInsets.only(right: theme.smallSpacing, top: theme.tinySpacing, bottom: theme.tinySpacing),
    ),
    endChild: children[index],
    beforeLineStyle: LineStyle(thickness: 1, color: theme.themeData.colorScheme.secondary),
  );
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [for (int i = 0; i < children.length; i++) _buildItem(theme, i)]);
  }
}
